...
namespace SBX_ERP
{
    public partial class frm_terceros : Form
    {
        ...
        {
            frm_vehiculos frm_Vehiculos = new frm_vehiculos();
            frm_Vehiculos.Show();
        }
        ...
        private void btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            if (v_contador == 0)
            {
                ...
                cls_Terceros.v_buscar = txt_identificacion.Text;
                v_dt = cls_Terceros.mtd_consultar();
                if (v_dt.Rows.Count > 0)
                {
                    //Editar
                    cls_Terceros.v_buscar = txt_identificacion.Text;

                    cls_Terceros.cl_identificacion = txt_identificacion.Text;
                    cls_Terceros.cl_tipo_identificacion = cbx_tipo_identificacion.SelectedIndex.ToString();
                    cls_Terceros.cl_razon_social = txt_razon_social.Text;
                    cls_Terceros.cl_tipo_persona = cbx_tipo_persona.SelectedIndex.ToString();
                    cls_Terceros.cl_direccion = txt_direccion.Text;
                    cls_Terceros.cl_telefonos = txt_telefonos.Text;
                    cls_Terceros.cl_Email = txt_email.Text;
                    ...
                    if (v_ok)
                    {
                        MessageBox.Show("Tercero Editado correctamente", "Exitoso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        mtd_limpiar();
                    }
                }
                else
                {
                    ...
                    if (v_ok)
                    {
                        MessageBox.Show("Tercero registrado correctamente", "Exitoso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        mtd_limpiar();
                    }
                }
            }           
